...
struct tas5805m_priv {
	...
	struct gpio_desc 		*gpio_bias_monitor;
                                   ...
                                   enum snd_soc_bias_level level)
{
    ...
    if (tas5805m->gpio_bias_monitor) {
        if (level == SND_SOC_BIAS_ON)
            gpiod_set_value(tas5805m->gpio_bias_monitor, 1);
        else
...
static int tas5805m_i2c_probe(struct i2c_client *i2c)
{
	...
	tas5805m->gpio_bias_monitor = devm_gpiod_get_optional(dev, "bias-monitor", GPIOD_OUT_LOW);
	if (IS_ERR(tas5805m->gpio_bias_monitor)) {
	    dev_err(dev, "error requesting bias-monitor gpio: %ld\n",
		    	PTR_ERR(tas5805m->gpio_bias_monitor));
	    return PTR_ERR(tas5805m->gpio_bias_monitor);
	}
